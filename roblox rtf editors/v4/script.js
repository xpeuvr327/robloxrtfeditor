...
// Définir une fonction qui applique un style au texte en fonction de la case à cocher
function applyStyle(style) {
  ...
  // Récupérer l'élément checkbox qui correspond au style
  let checkbox = document.getElementById(style);
  ...
  if (checkbox.checked) {
    ...
    // Enlever le style du texte
    let transformedText = input.value.replace(styles[style], "").replace(styles[style].replace("<", "</"), "");
    // Afficher le texte transformé dans l'élément output
    output.value = transformedText;
  }
}
